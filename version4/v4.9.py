...
class StockMarketGame:
    def __init__(self):
        ...
        self.investor_arrival_timer = random.randint(50, 150)  # Time until next investor arrives
        self.base_investment_amount = 10000  # Base investment amount
        ...
        for investor, (initial_investment, start_turn) in list(self.investors.items()):
            ...
            if random.random() < withdrawal_chance:
                if self.cash >= owed_amount:
                    ...
                    if self.irs_percentage - 1 < 0:
                        self.irs_percentage = 0
                    else:
                        self.irs_percentage -= 1
                else:
                    self.message = f"Not enough cash to pay {investor}!"
                    self.irs_percentage +=0.2
    ...
    def add_investor(self):
        ...
        if available_investors:
            ...
            investment_amount = self.base_investment_amount * random.uniform(0.8, 1.5)  # Vary the amount
    ...
    def calculate_owed_percentage(self, years):
        if years >= 7:
            return 1.40  # 80% after 5 years
        elif years >= 5:
            return 1.20  # 40% after 2 years
        return 0  # No additional amount owed
    def irs_meter(self):
        global play, game_over


        if self.irs_percentage >= 40:
            self.message = ("The IRS is getting suspicious")
        if self.irs_percentage >= 75:
            self.message = ("The IRS might show up soon...")
            self.cash == 0.75*self.cash
        if self.irs_percentage >= 100:
            self.message = ("You have been found guilty on 100 counts of tax fraud over your investment career. YOU ARE COOKED...GAME OVER!!!!!!!!")
            play = False
            game_over = True
    
    def display_irs_bar(self):
        # Color configuration
        START_COLOR = (0, 255, 0)  # Bright green
        END_COLOR = (255, 0, 0)    # Bright red
        BORDER_COLOR = (0, 0, 0)   # Subsegment border color

        # Bar dimensions
        bar_x, bar_y = 750, 500
        bar_width, bar_height = 200, 35
        chunks = 10  # Number of color chunks
        chunk_width = bar_width / chunks
        
        # Calculate color steps
        color_step = [
            (
                int(START_COLOR[0] + (END_COLOR[0] - START_COLOR[0]) * (i/chunks)),
                int(START_COLOR[1] + (END_COLOR[1] - START_COLOR[1]) * (i/chunks)),
                int(START_COLOR[2] + (END_COLOR[2] - START_COLOR[2]) * (i/chunks))
            )
        ...
        # Draw text label
        text = "IRS DANGER BAR"
        textInstance = font.render(text, True, END_COLOR)
        screen.blit(textInstance, (bar_x, bar_y - 25))
        
        # Draw main bar container
        pygame.draw.rect(screen, BORDER_COLOR, [bar_x, bar_y, bar_width, bar_height], 2)

        # Calculate filled chunks
        filled_chunks = int(self.irs_percentage / (100/chunks))
        partial_chunk = (self.irs_percentage % (100/chunks)) / (100/chunks)

        # Draw filled chunks
        for i in range(filled_chunks):
            chunk_x = bar_x + i * chunk_width
            # Full chunk fill
            try:
                pygame.draw.rect(screen, color_step[i], 
                                [chunk_x+2, bar_y+2, chunk_width-2, bar_height-4])
            except:
                pygame.draw.rect(screen, color_step[9], 
                                [chunk_x+2, bar_y+2, chunk_width-2, bar_height-4])
                break

            # Right border
            pygame.draw.line(screen, BORDER_COLOR,
                            (chunk_x + chunk_width, bar_y+1),
                            (chunk_x + chunk_width, bar_y + bar_height-2), 2)

        # Draw partial chunk
        if filled_chunks < chunks:
            partial_width = chunk_width * partial_chunk
            chunk_x = bar_x + filled_chunks * chunk_width
            pygame.draw.rect(screen, color_step[filled_chunks], 
                            [chunk_x+2, bar_y+2, partial_width, bar_height-4])
            # Right border
            pygame.draw.line(screen, BORDER_COLOR,
                            (chunk_x + partial_width, bar_y+1),
                            (chunk_x + partial_width, bar_y + bar_height-2), 2)

        # Draw left border for first chunk
        pygame.draw.line(screen, BORDER_COLOR,
                        (bar_x+1, bar_y+1),
                        (bar_x+1, bar_y + bar_height-2), 2)
    def irs_handler(self):
        pass
...
while game_running:
    ...
    if game_instance.irs_percentage < 100:
        game_instance.irs_percentage += 0.0
    game_instance.irs_meter()
    game_instance.display_irs_bar()
    # if popup:
    #     popup = open_popup(screen, "Smelly")
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if buy_button.is_clicked(pos) and not game_over:
            ...
            if sell_button.is_clicked(pos) and not game_over:
                game_instance.sell_stock(input_box.text)
                input_box.text = ''  # Clear the input box after selling
                input_box.txt_surface = font.render(input_box.text, True, input_box.color)  # Update rendered text
            if hold_button.is_clicked(pos) and not game_over:
